...
Let's start by moving the `amdahl` directory form our home directory to our scratch directory to save our valuable home directory space:
```bash
[NetID@log-1 ~]$ mv amdahl /scratch/$USER
```
:::tip
For more information about storage options on Greene please see [HPC Storage](../03_storage/01_intro_and_data_management.mdx).
:::

Move into the extracted directory, then use pip, to install it in your (“user”) scratch directory:
```bash
[NetID@log-1 ~]$ cd /scratch/$USER/amdahl
...
```bash
[NetID@log-1 amdahl]$ amdahl --help
...
```bash
[NetID@log-1 amdahl]$ nano serial-job.sh
...
```
```bash
[NetID@log-1 amdahl]$ sbatch serial-job.sh
```
...
```bash
[NetID@log-1 amdahl]$ sbatch parallel-job.sh
```
...
```bash
[NetID@log-1 amdahl]$ nano parallel-job.sh
...
```bash
[NetID@log-1 amdahl]$ for n in 30.033 10.888 7.697; do python3 -c "print(30.033 / $n)"; done
```
