...
If you have not already connected to Torch, please do so now:
```bash
...
Open a second terminal window on your local computer and run the `ls` command (without logging in to Torch). What differences do you see? <br />
**[Click for Solution]**
:::
</summary>
:::tip[Solution]
...
On Greene, you have a number of places where you can store your files. These differ in both the amount of space allocated and whether or not they are backed up.
...
If you want to transfer larger amounts of data to or from the cluster, Torch offers dedicated nodes for data transfers only. The motivation for this lies in the fact that larger data transfers should not obstruct operation of the login node for anybody else. As a rule of thumb, consider all transfers of a volume larger than 500 MB to 1 GB as large. But these numbers change, e.g., depending on the network connection of yourself and of your cluster or other factors.
:::
