...
-   Working with `conda` environments: To avoid running out of inode limits in home directories, the HPC team recommends **setting up `conda` environments with Apptainer overlay images** as [described here](../07_containers/03_apptainer_with_conda.md). Avoid creating `conda` environments in your `$HOME` directory.
:::
